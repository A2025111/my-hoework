...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define ENCODER_PULSES_PER_REV 52.0f 
#define REDUCTION_RATIO 20.409f
/* USER CODE END PD */
...
PID_t speed_pid = {13.0f, 8.0f, 0.5f, 0.0f, 0.0f, 0.0f};
...
void Calculate_Speed(void)
{
    ...
    if(current_time - last_calc_time >= 50)
    {
        ...
        // 计算电机转子转速 (转/秒)
        float motor_rotor_speed = (count_diff / ENCODER_PULSES_PER_REV) / (0.05f);
        
        actual_speed = motor_rotor_speed / REDUCTION_RATIO;
...
void VOFA_SendData(float target, float actual, float pid_adjust,uint32_t pwm, float error)
{
    printf("%.2f,%.2f,%.2f,%.2f,%.2f\n", target, actual, pid_adjust,(float)pwm, error);
}
void PID_Control(void)
{
    Update_Encoder_Count();
    Calculate_Speed();
    float pid_adjust = Incremental_PID_Calculate(target_speed, actual_speed);//pid增量
    int32_t new_pwm = pwm_value + (int32_t)pid_adjust;
    Set_Motor_PWM(Constrain(new_pwm, 0, 999));
   vofa_send_counter++;
    if (vofa_send_counter >= 5) 
	{
        VOFA_SendData(target_speed, actual_speed,pid_adjust, pwm_value, target_speed - actual_speed);
        vofa_send_counter = 0;
    }
}
